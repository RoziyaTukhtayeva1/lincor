...
import { GET_STUDENTS, DELETE_STUDENTS } from "../../redux/actions/types";
...
const Contact = () => {
  ...
  useEffect(() => {
    dispatch(getAction("users/statistika", GET_STUDENTS));
  }, [dispatch]);
console.log(data)
  const data1 = [];
const confirm = ()=>{
  dispatch(deleteAction("users/admin/delete", DELETE_STUDENTS));
  ...
  const dataMap = data?.allUsers.map(
    (d, index) =>
      index >= minIndex &&
      index < maxIndex && (
        <tr key={d.index}>
          <td>{++index}</td>
          <td>{d.first_name} {d?.last_name ? d?.last_name[0] : d?.last_name}.</td>
          <td>{d.email}</td>
          <td>
